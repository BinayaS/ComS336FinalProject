class player {
  ...
  update(objectList,delta) {
    ...
    if(rotation > 360) {
      this.mouse.yawObject.rotation.y = 0;
      ...
      //-- onGround Movement Calculation --
      {
          ...
          if(objectList.length > 0) {
            ...
            //-- Itterate Through the list --
            {
              for(var i = 0; i < objectList.length; i++) {
                ...
                {
                  if(placeMeeting( this.mouse.box.min.x + this.hspdX, this.mouse.box.max.x + this.hspdX,
                                    this.mouse.box.min.z, this.mouse.box.max.z,
                                    this.mouse.box.min.y, this.mouse.box.max.y, objectList[i])) {

                    if(placeMeeting( this.mouse.box.min.x + sign(this.hspdX), this.mouse.box.max.x + sign(this.hspdX),
                                      this.mouse.box.min.z, this.mouse.box.max.z,
                                      this.mouse.box.min.y, this.mouse.box.max.y, objectList[i])) {
                ...
                {
                  if(placeMeeting( this.mouse.box.min.x, this.mouse.box.max.x,
                                    this.mouse.box.min.z + this.hspdZ, this.mouse.box.max.z + this.hspdZ,
                                    this.mouse.box.min.y, this.mouse.box.max.y, objectList[i])) {

                    if(placeMeeting( this.mouse.box.min.x, this.mouse.box.max.x,
                                      this.mouse.box.min.z + sign(this.hspdZ), this.mouse.box.max.z + sign(this.hspdZ),
                                      this.mouse.box.min.y, this.mouse.box.max.y, objectList[i])) {
                ...
                //-- Check for collision in the y axis --
                {
                  if(placeMeeting( this.mouse.box.min.x, this.mouse.box.max.x,
                                    this.mouse.box.min.z, this.mouse.box.max.z,
                                    this.mouse.box.min.y + this.vspd, this.mouse.box.max.y + this.vspd, objectList[i])) {

                    if(placeMeeting( this.mouse.box.min.x, this.mouse.box.max.x,
                                      this.mouse.box.min.z, this.mouse.box.max.z,
                                      this.mouse.box.min.y+sign(this.vspd), this.mouse.box.max.y+sign(this.vspd), objectList[i])) {
        ...
        //-- Check if there still is a collision in the x axis --
        {
          if(this.collisionXObject != null) {
            if(placeMeeting(this.mouse.box.min.x + this.hspdX, this.mouse.box.max.x + this.hspdX,
                            this.mouse.box.min.z, this.mouse.box.max.z,
                            this.mouse.box.min.y, this.mouse.box.max.y, this.collisionXObject)) {
                            this.collisionX = false;
                            //this.collisionXObject = null;
            } else if(!placeMeeting(this.mouse.box.min.x + sign(this.hspdX), this.mouse.box.max.x + sign(this.hspdX),
                            this.mouse.box.min.z, this.mouse.box.max.z,
                            this.mouse.box.min.y, this.mouse.box.max.y, this.collisionXObject)) {
                            this.collisionXSign = false;
                            this.collisionXObject = null;
        ...
        //-- Check if there still is a collision in the x axis --
        {
          if(this.collisionZObject != null) {
            if(placeMeeting( this.mouse.box.min.x, this.mouse.box.max.x,
                            this.mouse.box.min.z + this.hspdZ, this.mouse.box.max.z + this.hspdZ,
                            this.mouse.box.min.y, this.mouse.box.max.y, this.collisionZObject)) {
                            this.collisionZ = false;
                            //this.collisionZObject = null;
            } else if(!placeMeeting(this.mouse.box.min.x, this.mouse.box.max.x,
                            this.mouse.box.min.z + sign(this.hspdZ), this.mouse.box.max.z + sign(this.hspdZ),
                            this.mouse.box.min.y, this.mouse.box.max.y, this.collisionZObject)) {
                            this.collisionZSign = false;
                            this.collisionZObject = null;
          ...
          if(this.collisionYObject != null) {
            if(this.vspd==0){
              ...
              this.collisionYObject = null;
                            ...
                            this.mouse.box.min.z, this.mouse.box.max.z,
                            this.mouse.box.min.y + this.vspd, this.mouse.box.max.y + this.vspd, this.collisionYObject)) {
                            this.collisionY = false;
                            //this.collisionYObject = null;
            } else if(!placeMeeting(this.mouse.box.min.x, this.mouse.box.max.x,
                            this.mouse.box.min.z, this.mouse.box.max.z,
                            this.mouse.box.min.y + sign(this.vspd), this.mouse.box.max.y + sign(this.vspd), this.collisionYObject)) {
                            this.collisionYSign = false;
                            this.collisionYObject = null;
            }
          }
    ...
    //-- Debug Code --
    {
      ...
      //console.log(this.mouse.box.min);
